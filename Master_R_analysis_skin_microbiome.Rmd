...
#Sort abund_all table by the 20 most abundant accross all sample types, after converting counts to relative abundance (%).
...
ggplot(top20, aes(x=variable, y=value, fill=OTU))+geom_bar(stat="identity")+scale_fill_hue(l=50) + labs(title="Top 20 OTUs across all sample types", x="", y="Relative abundance (%)") + guides(fill=guide_legend(keywidth=0.5, keyheight=0.5))
...
#Now plot a stacked barplot
ggplot(top20skin, aes(x=variable, y=value, fill=OTU))+geom_bar(stat="identity")+scale_fill_hue(l=50)+ labs(title="Top 20 OTUs in control and diabetic skin", x="", y="Relative abundance (%)")+ guides(fill=guide_legend(keywidth=0.5, keyheight=0.5))
#Repeat for top 10
...
###Basic Random Forest
```{r}
wh0 = genefilter_sample(skin_vst, filterfun_sample(function(x) x > 0), A = 0.10 * nsamples(skin_vst))
rare_skin_filter <- prune_taxa(wh0, skin_vst)
```{r,echo=FALSE}
